...
def on_key_press(key, stop_event,beacon2_stop,beacon3_stop,tag_stop):
    try:
        ...
        pass
# checks if there is a connection possible
def both_wrong_measurements(beacon_number):
    pass
...
def check_inputs(beacon_number, beacon_dist, tag_dist):
    if -5 <= (beacon_dist - tag_dist) <= 25:                    # correct case
    ...
    if 0 <= (beacon_dist - avg_distances[beacon_number]) <= 20: # tag wrong
        ...
        if tag_connection[beacon_number*2+1] == True:           # last time tag was already not working
            ...
            return True
        beacon_connection[beacon_number*2+1] = False            # first time tag not working
...
def find_sensor(ser_send, ser_receive, port_nr_1, port_nr_2):
    ...
    for index in range(1, max_sensors):
        ...
        for other_pair in pairs:
            if 5 <= dist1 - other_pair[0] <= 20: return index,other_pair[2]
            if 5 <= other_pair[1] - dist2 <= 20: return other_pair[2],index
