...
def initialize_global_variables():
    ...
    global board_type, port0,port1,port2, port_and_sensors_list, sketch_path
    global data_queue
    ...
    port1 = '/dev/ttyACM1'
    port2 = '/dev/ttyACM2'
    port_and_sensors_list = [('/dev/ttyACM0', 2), ('/dev/ttyACM1', 2), ('/dev/ttyACM2', 2)]
...
# measure the distances between the beacons
def initialize_beacons(sensor_queues, beacons_info):
    max_sensors = 2            
    num_iterations = 3
    num_beacons = len(sensor_queues)
    average_distances = [[0.0] * num_beacons for _ in range(num_beacons)]
    successful_measurements = [[0] * num_beacons for _ in range(num_beacons)]
    for iteration in range(num_iterations):
        for beacon1, beacon2 in itertools.combinations(range(num_beacons), 2):
            min1 = float('inf') # used to see what sensor measures the smallest dist
            min2 = float('inf')
            for x in range(1, max_sensors + 1):
                sensor_queues[beacon1].put(str(x))
                sensor_queues[beacon2].put(str(x))
                time.sleep(0.3)
                try:
                    distance_out = float(beacons_info[beacon1].get(block=False))
                    print(f"Queue went correctly, {beacon1}->{beacon2}: {distance_out}")
                    if distance_out<min1: min1 = distance_out
                except queue.Empty:
                    print(f"No data found in beacon{beacon1} queue")
                try:
                    distance_in = float(beacons_info[beacon2].get(block=False))
                    print(f"Queue went correctly, {beacon2}->{beacon1}: {distance_in}")
                    if distance_in<min2: min2 = distance_in
                except queue.Empty:
                    print(f"No data found in beacon{beacon2} queue")
            average_distances[beacon1][beacon2] += min1
            successful_measurements[beacon1][beacon2] += 1
            average_distances[beacon2][beacon1] += min2
            successful_measurements[beacon2][beacon1] += 1
    print("The average distances are:")
    for i in range(num_beacons):
        for j in range(i + 1, num_beacons):
            average_distance_out = (average_distances[i][j] / successful_measurements[i][j]) * 343 / 10000
            average_distance_in = (average_distances[j][i] / successful_measurements[j][i]) * 343 / 10000
            print(f"{i} to {j}: {average_distance_out}")
            print(f"{j} to {i}: {average_distance_in}")

    # avg_dist_12 = (average_distances[0][1]+average_distances[1][0]) / 2 / 3 * 343 / 10000 # assuming succeful measurements is 3
    # avg_dist_13 = (average_distances[0][2]+average_distances[2][0]) / 2 / 3 * 343 / 10000 # assuming succeful measurements is 3
    
    # known_locations[1] = [0,avg_dist_12,0]
    
    # direction_vector = (known_locations[1] - known_locations[0]) / np.linalg.norm(known_locations[1] - known_locations[0])
    # known_locations[2] = avg_dist_13 * direction_vector

    # # Calculate the angle between vectors AB and AC using the dot product
    # cos_angle = np.dot(direction_vector, np.array([1, 0, 0]))  # Assuming AB vector is along x-axis
    # angle = np.arccos(cos_angle)

    # # Rotate the direction vector by the calculated angle to find the position of C
    ...
    # # Calculate the position of the third beacon in the plane
    # known_locations[2] = avg_dist_13 * direction_vector_rotated


    # print(f"Locaton beacon 2:{known_locations[1]}")
    # print(f"Locaton beacon 3:{known_locations[2]}")
# starts all threads
def start_all_threads(csv_writer):
    ...
    beacons_info = [queue.Queue() for _ in port_and_sensors_list]   # make a beacons queue for each port
    for port_info,sensor_queue,beacon_info in zip(port_and_sensors_list, sensor_queues, beacons_info):# make all threads for the ports
        thread = threading.Thread(target=measurements_for_port, args=(csv_writer,stop_event,port_info,csv_offset,sensor_queue,beacon_info,))
        csv_offset += port_info[1]  # update the offset
        threads.append(thread)
    for thread in threads:          # Start all measurement threads
        thread.start()  
    time.sleep(1)
    initialize_beacons(sensor_queues,beacons_info)
    sc_static = ax.scatter([], [], [], color='blue')  # The beacons
    sc_static._offsets3d = tuple(map(tuple, known_locations.T))
...
def measurements_for_port(csv_writer,stop_event,port_info,csv_offset,sensor_queue,beacon_info):
    ...
    try:
        while not stop_event.is_set(): # run until stopted                
            ...
            if serialInst.in_waiting:   # wait for data from the arduino
                ...
                numeric_part = data.split(":")[1].strip()
                beacon_info.put(numeric_part)
