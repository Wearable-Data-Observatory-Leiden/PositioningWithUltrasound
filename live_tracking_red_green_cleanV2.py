...
def initialize_global_variables():
    ...
    global known_locations, avg_distances, threshold, start_time
...
def find_tag(tag_stop):
    ...
    while not tag_stop.is_set():
        ...
        within_range1, average = within_threshold(dist1t,distt1)
        if (within_range1): avg_distances[0] = average
        within_range2, average = within_threshold(dist2t,distt2)
        if (within_range2): avg_distances[1] = average
        within_range3, average = within_threshold(dist3t,distt3)
        if (within_range3): avg_distances[2] = average
