...
def initialize_global_variables():
    ...
    global port0,port1,port2,port3, beacon_connection, tag_connection
...
def trilateration(r1,r2,r3):
    ...
    K1 = np.round(P1 + X * Xn + Y * Yn + Z1 * Zn).astype(int)
    K2 = np.round(P1 + X * Xn + Y * Yn + Z2 * Zn).astype(int)
...
def trigger_all_ports(beacon_sensor, tag_sensor):
    ...
    try:
        ...
        return dist_b1,dist_b2,dist_b3
...
def find_tag(tag_stop):
    while not tag_stop.is_set():
        beacon1s1,beacon2s1,beacon3s1 = trigger_all_ports("1","1")
        new_point = trilateration(beacon1s1,beacon2s1,beacon3s1)
        print(f"New point in find tag: {new_point}")
        data_queue.put(new_point)
        #beacon1s2,beacon2s2,beacon3s2 = trigger_all_ports("2","1")
# starts all threads
