...
def initialize_global_variables():
    ...
    global data_queue, time_to_cm, serialInst1, serialInst2, serialInst3
...
# keyboard listener
def on_key_press(key, stop_event,beacon2_stop,beacon3_stop):
    try:
        letter = key.char
...
def initalize_beacon3(beacon3_stop):
    ...
    beacon3_x = ((avg_13 ** 2) - (avg_23 ** 2) + (known_locations[2][1] ** 2)) / (2*known_locations[2][1])
...
def start_all_threads():
    ...
    keyboard_listener = keyboard.Listener(on_press=lambda key: on_key_press(key, stop_event,beacon2_stop,beacon3_stop))
    ...
    for thread in threads:          # Start all measurement threads
        thread.start()  
    time.sleep(2)
    initalize_beacon2(beacon2_stop)
