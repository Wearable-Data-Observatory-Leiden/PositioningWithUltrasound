...
data_sensor_pairs.to_csv('test1_sensors_init.csv')
...
for x in range(0,36):
    ...
    tag = order[x][1]
    data_beacon = data_sensor_pairs.loc[start_row:end_row-1, beacon]
    ...
    data_tag = data_sensor_pairs.loc[start_row:end_row-1, tag]
...
print(df_sensors)
df_sensors.to_csv('results1_difference.csv')
...
df_beacon_mean.to_csv('results1_beacons_mean.csv')
...
df_tag_mean.to_csv('results1_tag_mean.csv')
...
print(df_avg)
df_avg.to_csv('results1_avg_beacon+tag/2.csv')



# this is where the tracking of the tag starts
data_tracking = pd.read_csv(csv_file_path, skiprows=indexes[35], header=None, delimiter=',', usecols=range(13))
data_tracking.to_csv('test1_tracking.csv')
headers = header_line.split(',')[:13]
